from load_trades import load_trades_from_json, Trade
from trade_aggregator import position_aggregations, delta_exposure, TradePositionAggregation, DeltaExposure, find_p_n_l
...
def test_average_price(aggregated_trades):
    ...
    trade_agg = next(
        (ta for ta in aggregated_trades 
...
def test_number_aggregated_trades(aggregated_trades):
    ...
    trade_agg = [ta for ta in aggregated_trades 
         if ta.delivery_period=='2026-01' 
         and ta.book =='physical'
         and ta.commodity=='nbp_gas']
...
def test_delta_exposure(trade_list):
    ...
    trade_de = next(
        (de for de in deltas 
         if de.delivery_period=='2026-01' 
         and de.commodity =='nbp_gas'),
        None,
    )
...
def test_position_aggregation(aggregated_trades): 
    ...
    jan_phys = next(at for at in aggregated_trades
...
def test_pnl_total(aggregated_trades, static_price_provider):
    ...
    for at in aggregated_trades:
        pnl = find_p_n_l(aggregated_trade=at,
